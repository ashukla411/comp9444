...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    #...
    #Single layer code
    # w = tf.Variable(tf.random_normal([state_dim, action_dim]))
    # b = tf.Variable(tf.random_normal([action_dim]))
    # single_layer = tf.matmul(state_in, w) + b
    # q_values = single_layer

    w1 = tf.Variable(tf.random_normal( [ state_dim, hidden_nodes]))
    b1 = tf.Variable(tf.random_normal([hidden_nodes]))
    logits = tf.matmul(state_in, w1) + b1
    pred1 = tf.nn.tanh(logits)
    w2 = tf.Variable(tf.zeros([hidden_nodes, action_dim]))
    b2 = tf.Variable(tf.zeros([action_dim]))
    two_layer = tf.matmul(pred1, w2) + b2
    q_values = two_layer
...
def get_train_batch(q_values, state_in, replay_buffer):
    ...
    # print("===============================================================================================")
    # print(Q_value_batch)
    # print(state_batch,action_batch,reward_batch,next_state_batch,Q_value_batch)
...
def main():
    ...
    #reset this to render=TRUE
    qtrain(env, state_dim, action_dim, *network_vars, render=RENDER)
